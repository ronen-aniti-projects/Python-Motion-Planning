"""
This module is an implementation of the RRT algorithm.
"""
...
class RRT:
    ...
    def __init__(
        ...
        self._MAX_STEER_ANGLE = (
            MAX_STEER_ANGLE_RATE * TIME_STEP
        ...
        self._INTEGRATION_STEPS = int(
            self._TIME_INTERVAL / self._TIME_STEP
        )  # Iterations of each branch expansion process
        self._STEERING = True
        ...
        if not inside_environment(self._environment, self._start_pos):
            raise ValueError(
                "Start position is outside of the bounds of the environment"
            )
        if not inside_environment(self._environment, self._goal_pos):
            raise ValueError(
                "Goal position is outside of the bounds of the environment"
            )
    ...
    def sample_with_bias(self):
        ...
        if random_number < self._GOAL_BIAS:
            ...
            p_sample = np.array(
                [
                    np.random.uniform(
                        self._environment.xbounds[0],
                        self._environment.xbounds[1],
                    ),
                    np.random.uniform(
                        self._environment.ybounds[0],
                        self._environment.ybounds[1],
                    ),
                    np.random.uniform(
                        self._environment.zbounds[0],
                        self._environment.zbounds[1],
                    ),
                ]
            )
        return p_sample
    ...
    def integrate_forward(self, x_near, p_sample):
        ...
        if not self._STEERING:
            return x_i + 5.0 * (p_sample - x_near[:3]) / np.linalg.norm(
                p_sample - x_near[:3]
            )
    ...
    def update_state(self, x_near, p_sample):
        ...
        # Otherwise, calculate the rotation matrix using Rodrigues' formula
            ...
            theta = np.arccos(
                (np.dot(prior_attitude, u_1))
                / (np.linalg.norm(u_1) * np.linalg.norm(prior_attitude))
            )
            ...
            R = (
                np.eye(3)
                + np.sin(steer_angle) * K
                + (1 - np.cos(steer_angle)) * np.dot(K, K)
            )
    ...
    def visualize(self, env_obj, path=[]):
        ...
        def boxes_intersect(center, halfsize, min_bound, max_bound):
            return all((center - halfsize) < max_bound) and all(
                (center + halfsize) > min_bound
            )
